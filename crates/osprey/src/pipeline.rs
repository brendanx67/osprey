...
fn run_calibration_discovery_windowed(
    ...
    // bins (~2K) -- calibration only needs to discriminate target/decoy and
    // the unit-bin XCorr is dramatically cheaper than HRAM (2001 vs 100K
    // bins per spectrum). Matches the intent in
    // run_xcorr_calibration_scoring's "Always use unit resolution bins for
    // calibration XCorr" comment, kept in sync with OspreySharp's
    // s_calXcorrScorer. Main search (run_search) still uses the
    // resolution-mode bins.
