...
pub struct RTCalibratorConfig {
    ...
    /// single refinement regardless of `robustness_iter >= 1`. When true, each
    /// iteration recomputes residuals from the current fit, which is the
    /// classical Cleveland (1979) robust LOESS algorithm. Default false keeps
    /// backward compatibility; both this crate and the OspreySharp C# port
    /// honor the OSPREY_LOESS_CLASSICAL_ROBUST=1 env var to flip to classical
    /// mode in sync for cross-implementation validation.
...
impl RTCalibrator {
    ...
    pub fn fit(&self, library_rts: &[f64], measured_rts: &[f64]) -> Result<RTCalibration> {
        ...
        // every iteration of the robustness loop — this is what historical
        // Osprey has always done, and produces a single refinement regardless
        // of `robustness_iter >= 1`. In classical mode (Cleveland 1979),
